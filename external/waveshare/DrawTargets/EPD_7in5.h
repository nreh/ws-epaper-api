...
class EPD_7in5_DrawTarget : public Black1BitEPD {
  public:
    ...
    void Init() { controller::EPD_7IN5_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_7IN5_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_7IN5_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_7IN5_Display(packedBits); }
    ...
    EPD_7in5_DrawTarget(bool initialize = true) : Black1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
    ~EPD_7in5_DrawTarget() {}
