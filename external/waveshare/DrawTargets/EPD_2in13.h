...
class EPD_2in13_DrawTarget : public Black1BitEPD {
  public:
    ...
    void Clear() { controller::EPD_2IN13_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_2IN13_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_2IN13_Display(packedBits); }
    ...
    EPD_2in13_DrawTarget(bool initialize = true) : Black1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
