...
class EPD_1in54_DES_DrawTarget : public Black1BitEPD {
  public:
    ...
    void Init() { controller::EPD_1IN54_DES_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_1IN54_DES_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_1IN54_DES_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_1IN54_DES_Display(packedBits); }
    ...
    EPD_1in54_DES_DrawTarget(bool initialize = true) : Black1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
