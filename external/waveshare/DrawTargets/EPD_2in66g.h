...
class EPD_2in66g_DrawTarget : public Color2BitEPD {
  public:
    ...
    void Init() { controller::EPD_2IN66g_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_2IN66g_Clear(0x1); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_2IN66g_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_2IN66g_Display(packedBits); }
    ...
    EPD_2in66g_DrawTarget(bool initialize = true) : Color2BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
