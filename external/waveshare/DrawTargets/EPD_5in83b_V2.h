...
class EPD_5in83b_V2_DrawTarget : public RedBlack1BitEPD {
  public:
    ...
    void Init() { controller::EPD_5IN83B_V2_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_5IN83B_V2_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_5IN83B_V2_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_5IN83B_V2_Display(packedBitsBlack, packedBitsRed); }
    ...
    EPD_5in83b_V2_DrawTarget(bool initialize = true) : RedBlack1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
