...
class EPD_2in66b_DrawTarget : public RedBlack1BitEPD {
  public:
    ...
    void Init() { controller::EPD_2IN66B_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_2IN66B_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_2IN66B_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_2IN66B_Display(packedBitsBlack, packedBitsRed); }
    ...
    EPD_2in66b_DrawTarget(bool initialize = true) : RedBlack1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
