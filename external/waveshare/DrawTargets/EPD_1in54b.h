...
class EPD_1in54b_DrawTarget : public RedBlack1BitEPD {
  public:
    ...
    void Init() { controller::EPD_1IN54B_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_1IN54B_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_1IN54B_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_1IN54B_Display(packedBitsBlack, packedBitsRed); }
    ...
    EPD_1in54b_DrawTarget(bool initialize = true) : RedBlack1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
