...
class EPD_2in13b_V4_DrawTarget : public RedBlack1BitEPD {
  public:
    ...
    void Init() { controller::EPD_2IN13B_V4_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_2IN13B_V4_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_2IN13B_V4_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_2IN13B_V4_Display(packedBitsBlack, packedBitsRed); }
    ...
    EPD_2in13b_V4_DrawTarget(bool initialize = true) : RedBlack1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
