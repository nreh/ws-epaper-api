...
enum RefreshMode { Display = 0, Base = 1, Base_White = 2 };
...
class EPD_13in3b_DrawTarget : public RedBlack1BitEPD {
  public:
    ...
    //  ! EPD_13IN3B_Display_Partial(const UBYTE *Image, UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend) was skipped because I'm not sure what arguments to pass in!

    /// @brief Initialize the display
    void Init() {
        controller::EPD_13IN3B_Init();
    }

    /// @brief Clear the display
    void Clear() {
        controller::EPD_13IN3B_Clear();
    }

    /// @brief Clear the display
    void Clear_Red() {
        controller::EPD_13IN3B_Clear_Red();
    }

    /// @brief Clear the display
    void Clear_Black() {
        controller::EPD_13IN3B_Clear_Black();
    }

    /// @brief Put the display to sleep
    void Sleep() {
        controller::EPD_13IN3B_Sleep();
    }

    /// @brief Display pixels in buffers to display
    void Display() {
        controller::EPD_13IN3B_Display(packedBitsBlack, packedBitsRed);
    }

    /// @brief Display pixels in buffers to display
    void Display_Base() {
        controller::EPD_13IN3B_Display_Base(packedBitsBlack, packedBitsRed);
    }
    ...
    EPD_13in3b_DrawTarget(bool initializeSPI = true) : RedBlack1BitEPD(GetWidth(), GetHeight(), initializeSPI) {}
