...
class EPD_2in9bc_DrawTarget : public RedBlack1BitEPD {
  public:
    ...
    void Init() { controller::EPD_2IN9BC_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_2IN9BC_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_2IN9BC_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_2IN9BC_Display(packedBitsBlack, packedBitsRed); }
    ...
    EPD_2in9bc_DrawTarget(bool initialize = true) : RedBlack1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
