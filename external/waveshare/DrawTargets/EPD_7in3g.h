...
class EPD_7in3g_DrawTarget : public Color2BitEPD {
  public:
    ...
    void Init() { controller::EPD_7IN3G_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_7IN3G_Clear(0x1); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_7IN3G_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_7IN3G_Display(packedBits); }
    ...
    EPD_7in3g_DrawTarget(bool initialize = true) : Color2BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
