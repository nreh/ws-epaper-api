...
class EPD_10in2b_DrawTarget : public RedBlack1BitEPD {
  public:
    ...
    void Init() { controller::EPD_10IN2b_Init(); }

    /// @brief Clear the display
    void Clear() { controller::EPD_10IN2b_Clear(); }

    /// @brief Put the display to sleep
    void Sleep() { controller::EPD_10IN2b_Sleep(); }

    /// @brief Display pixels in buffers to display
    void Display() { controller::EPD_10IN2b_Display(packedBitsBlack, packedBitsRed); }
    ...
    EPD_10in2b_DrawTarget(bool initialize = true) : RedBlack1BitEPD(GetWidth(), GetHeight(), initialize) {
        if (initialize)
            Init();
    }
