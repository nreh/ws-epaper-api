...
/// @brief For internal use by EpaperEmulator
/// @param windowWidth Width of SDL window in pixels
/// @param windowHeight Height of SDL window in pixels
void InitializeSDL(int windowWidth, int windowHeight) {
    ...
    window = SDL_CreateWindow(
        "SDL Window", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, windowWidth, windowHeight, SDL_WINDOW_SHOWN
    );
    if (!window) {
...
class EpaperEmulator : public epaperapi::AbstractDrawTarget {
  public:
  ...
  private:
    ...
    void DrawPixels() {
        ...
        SDL_Rect rect;
        rect.x = windowWidth / 2 - (width / 2); // Centered on window
        rect.y = windowHeight / 2 - (height / 2);
        rect.w = width;
        rect.h = height;
    ...
    EpaperEmulator(epaperapi::devices::PhysicalEPDDrawTarget* _device, ColorMode _mode)
        ...
        windowWidth = _device->width + (2 * WINDOW_PADDING);
        windowHeight = _device->height + (2 * WINDOW_PADDING);
  ...
  public:
     ...
     */
    EpaperEmulator(uint16_t _width, uint16_t _height, ColorMode _mode = ColorMode::rgb)
        : epaperapi::AbstractDrawTarget(*(this->CreateBufferBasedOnColorMode(_width, _height, _mode))) {
        windowWidth = width + (2 * WINDOW_PADDING);
        windowHeight = height + (2 * WINDOW_PADDING);
        InitializeSDL(windowWidth, windowHeight);
        renderer = sdlrenderer;
        colormode = _mode;
    }
    ...
    /// is set to true.)
    template <typename Device> static EpaperEmulator CreatePhysical() {
        ...
        return EpaperEmulator(inst, mode);
    }
